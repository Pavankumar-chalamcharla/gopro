...
object SensorQualityAnalyzer {
    ...
    fun analyzeDynamicResponse(
        ...
        val gyroGrid = interpolateLinear(gyroSeries, grid)
        val rvGrid = interpolateLinear(rvSeries, grid)

        val maxLagSteps = (maxLagMs / gridStepMs).toInt()
        val lagResult = crossCorrelationLag(gyroGrid, rvGrid, maxLagSteps)
    ...
    /** Linear interpolation of a (time_ms, value) series onto arbitrary query times.
     *  Returns NaN for query points outside the series' time range. */
    internal fun interpolateLinear(series: List<Pair<Double, Double>>, queryTimesMs: DoubleArray): DoubleArray {
     ...
     * `a` leads `b` (see [DynamicResponseResult.bestLagMs] for the
     * physical interpretation). NaN entries (from interpolation gaps) are
     * excluded pairwise.
     */
    internal fun crossCorrelationLag(a: DoubleArray, b: DoubleArray, maxLagSteps: Int): LagResult {
    ...
    internal fun pearsonAtLag(a: DoubleArray, b: DoubleArray, lag: Int): Double {
        val n = a.size
